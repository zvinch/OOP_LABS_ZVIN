...
Node* myList::swapNodes2( Node* node1,  Node* node2) {
...
void myList::insert(Node & parent,const  Shape*  tobeadded){
...
void myList::add(const Shape * newcircle){
	this->insert(Head, newcircle);
}
void myList::append(const Shape * newcircle){
...
void myList::Selectionsort(bool (*compare)(const Shape* first, const Shape* second))
{
	...
	{
		Node* next_cur = cursor->next_node;
		swapNodes2(cursor, min(cursor, compare));
		cursor = next_cur;
		//std::cout << *this;
	}
...
Node * myList::min(Node *const  start, bool(*compare)(const Shape* first, const Shape* second))const
{
	Node* iter = start;
	Node* min = nullptr;
	int i = 0;
...
Node * myList::min( Node *const  start)const
{
	...
	{	
		...
		while (iter->next_node != nullptr)
		{
			if ( iter->data->Area() < min->data->Area())
			{
				min = iter;
			}
			iter = iter->next_node;
...
bool myList::seekanddestroyGREEDY(Shape * tobedeleted){
	Node * node_ptr = Head.next_node;
	while (node_ptr != &Tail)
	{
		if ((node_ptr)->data == tobedeleted)//virtual 
		{
			delete node_ptr;
			m_size--;
			return 1;
...
unsigned int myList::seekanddestroyALL(Shape * tobedeleted){
	Node * node_ptr = Head.next_node;
	unsigned int retval = 0;
	while (node_ptr != &Tail)
	{
		Node*  pn = node_ptr->next_node;
		if ((node_ptr)->data == tobedeleted)
		{
			delete node_ptr;
			m_size--;
			retval++;
...
std::ostream& operator<<(std::ostream& ostr, const myList& mlist)
{
	...
	while (np != &mlist.Tail){
		...
		ostr << rank << " -- ";
		if (Rect * p_tmp = dynamic_cast<Rect*> (np->data)){
			ostr << *p_tmp;
		}
		if (Circle * p_tmp = dynamic_cast<Circle*> (np->data)) {
			ostr << *p_tmp;
		}
			ostr << " this " << *(np) << " prev " << *(np->prev_node) << " next " << *(np->next_node) << std::endl;
		rank++;
		np = np->next_node;
	}
	ostr << "++++++++++++++++++++++++++++++" << std::endl;
...
myList & myList::operator = (const myList & other)
{
		...
		while (p_other != &other.Tail)
		{
			if (p_this != &Tail)
			{
				p_this->data = p_other->data;//
				p_this = p_this->next_node;
			}
			else
			{
				this->append(p_other->data);
